...
Node* buildTree(istream& is) {
	...
	op->right = buildTree(is); /* 연산자 노드의 경우 op가 nullptr가 아니므로, recursion을 통해 두 개의 자식 노드 만들어주기*/
...
int main(void) {
	...
	Node* root = buildTree(cin); /* 입력한 문자열을 pre-order traversal대로 tree 생성 후 root 노드 저장 */
